import torch
from torch import tensor
import torch.nn as nn
from torch.nn import *
import torchvision
import torchvision.models as models
from torchvision.ops.stochastic_depth import stochastic_depth
import time
import builtins
import operator

class M(torch.nn.Module):
    def __init__(self):
        super(M, self).__init__()
        self.conv2d0 = Conv2d(3, 192, kernel_size=(4, 4), stride=(4, 4))
        self.conv2d1 = Conv2d(192, 192, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=192)
        self.layernorm0 = LayerNorm((192,), eps=1e-06, elementwise_affine=True)
        self.linear0 = Linear(in_features=192, out_features=768, bias=True)
        self.gelu0 = GELU(approximate='none')
        self.linear1 = Linear(in_features=768, out_features=192, bias=True)
        self.conv2d2 = Conv2d(192, 192, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=192)
        self.layernorm1 = LayerNorm((192,), eps=1e-06, elementwise_affine=True)
        self.linear2 = Linear(in_features=192, out_features=768, bias=True)
        self.gelu1 = GELU(approximate='none')
        self.linear3 = Linear(in_features=768, out_features=192, bias=True)
        self.conv2d3 = Conv2d(192, 192, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=192)
        self.layernorm2 = LayerNorm((192,), eps=1e-06, elementwise_affine=True)
        self.linear4 = Linear(in_features=192, out_features=768, bias=True)
        self.gelu2 = GELU(approximate='none')
        self.linear5 = Linear(in_features=768, out_features=192, bias=True)
        self.conv2d4 = Conv2d(192, 384, kernel_size=(2, 2), stride=(2, 2))
        self.conv2d5 = Conv2d(384, 384, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=384)
        self.layernorm3 = LayerNorm((384,), eps=1e-06, elementwise_affine=True)
        self.linear6 = Linear(in_features=384, out_features=1536, bias=True)
        self.gelu3 = GELU(approximate='none')
        self.linear7 = Linear(in_features=1536, out_features=384, bias=True)
        self.conv2d6 = Conv2d(384, 384, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=384)
        self.layernorm4 = LayerNorm((384,), eps=1e-06, elementwise_affine=True)
        self.linear8 = Linear(in_features=384, out_features=1536, bias=True)
        self.gelu4 = GELU(approximate='none')
        self.linear9 = Linear(in_features=1536, out_features=384, bias=True)
        self.conv2d7 = Conv2d(384, 384, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=384)
        self.layernorm5 = LayerNorm((384,), eps=1e-06, elementwise_affine=True)
        self.linear10 = Linear(in_features=384, out_features=1536, bias=True)
        self.gelu5 = GELU(approximate='none')
        self.linear11 = Linear(in_features=1536, out_features=384, bias=True)
        self.conv2d8 = Conv2d(384, 768, kernel_size=(2, 2), stride=(2, 2))
        self.conv2d9 = Conv2d(768, 768, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=768)
        self.layernorm6 = LayerNorm((768,), eps=1e-06, elementwise_affine=True)
        self.linear12 = Linear(in_features=768, out_features=3072, bias=True)
        self.gelu6 = GELU(approximate='none')
        self.linear13 = Linear(in_features=3072, out_features=768, bias=True)
        self.conv2d10 = Conv2d(768, 768, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=768)
        self.layernorm7 = LayerNorm((768,), eps=1e-06, elementwise_affine=True)
        self.linear14 = Linear(in_features=768, out_features=3072, bias=True)
        self.gelu7 = GELU(approximate='none')
        self.linear15 = Linear(in_features=3072, out_features=768, bias=True)
        self.conv2d11 = Conv2d(768, 768, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=768)
        self.layernorm8 = LayerNorm((768,), eps=1e-06, elementwise_affine=True)
        self.linear16 = Linear(in_features=768, out_features=3072, bias=True)
        self.gelu8 = GELU(approximate='none')
        self.linear17 = Linear(in_features=3072, out_features=768, bias=True)
        self.conv2d12 = Conv2d(768, 768, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=768)
        self.layernorm9 = LayerNorm((768,), eps=1e-06, elementwise_affine=True)
        self.linear18 = Linear(in_features=768, out_features=3072, bias=True)
        self.gelu9 = GELU(approximate='none')
        self.linear19 = Linear(in_features=3072, out_features=768, bias=True)
        self.conv2d13 = Conv2d(768, 768, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=768)
        self.layernorm10 = LayerNorm((768,), eps=1e-06, elementwise_affine=True)
        self.linear20 = Linear(in_features=768, out_features=3072, bias=True)
        self.gelu10 = GELU(approximate='none')
        self.linear21 = Linear(in_features=3072, out_features=768, bias=True)
        self.conv2d14 = Conv2d(768, 768, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=768)
        self.layernorm11 = LayerNorm((768,), eps=1e-06, elementwise_affine=True)
        self.linear22 = Linear(in_features=768, out_features=3072, bias=True)
        self.gelu11 = GELU(approximate='none')
        self.linear23 = Linear(in_features=3072, out_features=768, bias=True)
        self.conv2d15 = Conv2d(768, 768, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=768)
        self.layernorm12 = LayerNorm((768,), eps=1e-06, elementwise_affine=True)
        self.linear24 = Linear(in_features=768, out_features=3072, bias=True)
        self.gelu12 = GELU(approximate='none')
        self.linear25 = Linear(in_features=3072, out_features=768, bias=True)
        self.conv2d16 = Conv2d(768, 768, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=768)
        self.layernorm13 = LayerNorm((768,), eps=1e-06, elementwise_affine=True)
        self.linear26 = Linear(in_features=768, out_features=3072, bias=True)
        self.gelu13 = GELU(approximate='none')
        self.linear27 = Linear(in_features=3072, out_features=768, bias=True)
        self.conv2d17 = Conv2d(768, 768, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=768)
        self.layernorm14 = LayerNorm((768,), eps=1e-06, elementwise_affine=True)
        self.linear28 = Linear(in_features=768, out_features=3072, bias=True)
        self.gelu14 = GELU(approximate='none')
        self.linear29 = Linear(in_features=3072, out_features=768, bias=True)
        self.conv2d18 = Conv2d(768, 768, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=768)
        self.layernorm15 = LayerNorm((768,), eps=1e-06, elementwise_affine=True)
        self.linear30 = Linear(in_features=768, out_features=3072, bias=True)
        self.gelu15 = GELU(approximate='none')
        self.linear31 = Linear(in_features=3072, out_features=768, bias=True)
        self.conv2d19 = Conv2d(768, 768, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=768)
        self.layernorm16 = LayerNorm((768,), eps=1e-06, elementwise_affine=True)
        self.linear32 = Linear(in_features=768, out_features=3072, bias=True)
        self.gelu16 = GELU(approximate='none')
        self.linear33 = Linear(in_features=3072, out_features=768, bias=True)
        self.conv2d20 = Conv2d(768, 768, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=768)
        self.layernorm17 = LayerNorm((768,), eps=1e-06, elementwise_affine=True)
        self.linear34 = Linear(in_features=768, out_features=3072, bias=True)
        self.gelu17 = GELU(approximate='none')
        self.linear35 = Linear(in_features=3072, out_features=768, bias=True)
        self.conv2d21 = Conv2d(768, 768, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=768)
        self.layernorm18 = LayerNorm((768,), eps=1e-06, elementwise_affine=True)
        self.linear36 = Linear(in_features=768, out_features=3072, bias=True)
        self.gelu18 = GELU(approximate='none')
        self.linear37 = Linear(in_features=3072, out_features=768, bias=True)
        self.conv2d22 = Conv2d(768, 768, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=768)
        self.layernorm19 = LayerNorm((768,), eps=1e-06, elementwise_affine=True)
        self.linear38 = Linear(in_features=768, out_features=3072, bias=True)
        self.gelu19 = GELU(approximate='none')
        self.linear39 = Linear(in_features=3072, out_features=768, bias=True)
        self.conv2d23 = Conv2d(768, 768, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=768)
        self.layernorm20 = LayerNorm((768,), eps=1e-06, elementwise_affine=True)
        self.linear40 = Linear(in_features=768, out_features=3072, bias=True)
        self.gelu20 = GELU(approximate='none')
        self.linear41 = Linear(in_features=3072, out_features=768, bias=True)
        self.conv2d24 = Conv2d(768, 768, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=768)
        self.layernorm21 = LayerNorm((768,), eps=1e-06, elementwise_affine=True)
        self.linear42 = Linear(in_features=768, out_features=3072, bias=True)
        self.gelu21 = GELU(approximate='none')
        self.linear43 = Linear(in_features=3072, out_features=768, bias=True)
        self.conv2d25 = Conv2d(768, 768, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=768)
        self.layernorm22 = LayerNorm((768,), eps=1e-06, elementwise_affine=True)
        self.linear44 = Linear(in_features=768, out_features=3072, bias=True)
        self.gelu22 = GELU(approximate='none')
        self.linear45 = Linear(in_features=3072, out_features=768, bias=True)
        self.conv2d26 = Conv2d(768, 768, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=768)
        self.layernorm23 = LayerNorm((768,), eps=1e-06, elementwise_affine=True)
        self.linear46 = Linear(in_features=768, out_features=3072, bias=True)
        self.gelu23 = GELU(approximate='none')
        self.linear47 = Linear(in_features=3072, out_features=768, bias=True)
        self.conv2d27 = Conv2d(768, 768, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=768)
        self.layernorm24 = LayerNorm((768,), eps=1e-06, elementwise_affine=True)
        self.linear48 = Linear(in_features=768, out_features=3072, bias=True)
        self.gelu24 = GELU(approximate='none')
        self.linear49 = Linear(in_features=3072, out_features=768, bias=True)
        self.conv2d28 = Conv2d(768, 768, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=768)
        self.layernorm25 = LayerNorm((768,), eps=1e-06, elementwise_affine=True)
        self.linear50 = Linear(in_features=768, out_features=3072, bias=True)
        self.gelu25 = GELU(approximate='none')
        self.linear51 = Linear(in_features=3072, out_features=768, bias=True)
        self.conv2d29 = Conv2d(768, 768, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=768)
        self.layernorm26 = LayerNorm((768,), eps=1e-06, elementwise_affine=True)
        self.linear52 = Linear(in_features=768, out_features=3072, bias=True)
        self.gelu26 = GELU(approximate='none')
        self.linear53 = Linear(in_features=3072, out_features=768, bias=True)
        self.conv2d30 = Conv2d(768, 768, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=768)
        self.layernorm27 = LayerNorm((768,), eps=1e-06, elementwise_affine=True)
        self.linear54 = Linear(in_features=768, out_features=3072, bias=True)
        self.gelu27 = GELU(approximate='none')
        self.linear55 = Linear(in_features=3072, out_features=768, bias=True)
        self.conv2d31 = Conv2d(768, 768, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=768)
        self.layernorm28 = LayerNorm((768,), eps=1e-06, elementwise_affine=True)
        self.linear56 = Linear(in_features=768, out_features=3072, bias=True)
        self.gelu28 = GELU(approximate='none')
        self.linear57 = Linear(in_features=3072, out_features=768, bias=True)
        self.conv2d32 = Conv2d(768, 768, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=768)
        self.layernorm29 = LayerNorm((768,), eps=1e-06, elementwise_affine=True)
        self.linear58 = Linear(in_features=768, out_features=3072, bias=True)
        self.gelu29 = GELU(approximate='none')
        self.linear59 = Linear(in_features=3072, out_features=768, bias=True)
        self.conv2d33 = Conv2d(768, 768, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=768)
        self.layernorm30 = LayerNorm((768,), eps=1e-06, elementwise_affine=True)
        self.linear60 = Linear(in_features=768, out_features=3072, bias=True)
        self.gelu30 = GELU(approximate='none')
        self.linear61 = Linear(in_features=3072, out_features=768, bias=True)
        self.conv2d34 = Conv2d(768, 768, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=768)
        self.layernorm31 = LayerNorm((768,), eps=1e-06, elementwise_affine=True)
        self.linear62 = Linear(in_features=768, out_features=3072, bias=True)
        self.gelu31 = GELU(approximate='none')
        self.linear63 = Linear(in_features=3072, out_features=768, bias=True)
        self.conv2d35 = Conv2d(768, 768, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=768)
        self.layernorm32 = LayerNorm((768,), eps=1e-06, elementwise_affine=True)
        self.linear64 = Linear(in_features=768, out_features=3072, bias=True)
        self.gelu32 = GELU(approximate='none')
        self.linear65 = Linear(in_features=3072, out_features=768, bias=True)
        self.conv2d36 = Conv2d(768, 1536, kernel_size=(2, 2), stride=(2, 2))
        self.conv2d37 = Conv2d(1536, 1536, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=1536)
        self.layernorm33 = LayerNorm((1536,), eps=1e-06, elementwise_affine=True)
        self.linear66 = Linear(in_features=1536, out_features=6144, bias=True)
        self.gelu33 = GELU(approximate='none')
        self.linear67 = Linear(in_features=6144, out_features=1536, bias=True)
        self.conv2d38 = Conv2d(1536, 1536, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=1536)
        self.layernorm34 = LayerNorm((1536,), eps=1e-06, elementwise_affine=True)
        self.linear68 = Linear(in_features=1536, out_features=6144, bias=True)
        self.gelu34 = GELU(approximate='none')
        self.linear69 = Linear(in_features=6144, out_features=1536, bias=True)
        self.conv2d39 = Conv2d(1536, 1536, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), groups=1536)
        self.layernorm35 = LayerNorm((1536,), eps=1e-06, elementwise_affine=True)
        self.linear70 = Linear(in_features=1536, out_features=6144, bias=True)
        self.gelu35 = GELU(approximate='none')
        self.linear71 = Linear(in_features=6144, out_features=1536, bias=True)
        self.adaptiveavgpool2d0 = AdaptiveAvgPool2d(output_size=1)
        self.flatten0 = Flatten(start_dim=1, end_dim=-1)
        self.linear72 = Linear(in_features=1536, out_features=1000, bias=True)
        self.weight0 = torch.rand(torch.Size([192]))
        self.bias0 = torch.rand(torch.Size([192]))
        self.layer_scale0 = torch.rand(torch.Size([192, 1, 1]))
        self.layer_scale1 = torch.rand(torch.Size([192, 1, 1]))
        self.layer_scale2 = torch.rand(torch.Size([192, 1, 1]))
        self.weight1 = torch.rand(torch.Size([192]))
        self.bias1 = torch.rand(torch.Size([192]))
        self.layer_scale3 = torch.rand(torch.Size([384, 1, 1]))
        self.layer_scale4 = torch.rand(torch.Size([384, 1, 1]))
        self.layer_scale5 = torch.rand(torch.Size([384, 1, 1]))
        self.weight2 = torch.rand(torch.Size([384]))
        self.bias2 = torch.rand(torch.Size([384]))
        self.layer_scale6 = torch.rand(torch.Size([768, 1, 1]))
        self.layer_scale7 = torch.rand(torch.Size([768, 1, 1]))
        self.layer_scale8 = torch.rand(torch.Size([768, 1, 1]))
        self.layer_scale9 = torch.rand(torch.Size([768, 1, 1]))
        self.layer_scale10 = torch.rand(torch.Size([768, 1, 1]))
        self.layer_scale11 = torch.rand(torch.Size([768, 1, 1]))
        self.layer_scale12 = torch.rand(torch.Size([768, 1, 1]))
        self.layer_scale13 = torch.rand(torch.Size([768, 1, 1]))
        self.layer_scale14 = torch.rand(torch.Size([768, 1, 1]))
        self.layer_scale15 = torch.rand(torch.Size([768, 1, 1]))
        self.layer_scale16 = torch.rand(torch.Size([768, 1, 1]))
        self.layer_scale17 = torch.rand(torch.Size([768, 1, 1]))
        self.layer_scale18 = torch.rand(torch.Size([768, 1, 1]))
        self.layer_scale19 = torch.rand(torch.Size([768, 1, 1]))
        self.layer_scale20 = torch.rand(torch.Size([768, 1, 1]))
        self.layer_scale21 = torch.rand(torch.Size([768, 1, 1]))
        self.layer_scale22 = torch.rand(torch.Size([768, 1, 1]))
        self.layer_scale23 = torch.rand(torch.Size([768, 1, 1]))
        self.layer_scale24 = torch.rand(torch.Size([768, 1, 1]))
        self.layer_scale25 = torch.rand(torch.Size([768, 1, 1]))
        self.layer_scale26 = torch.rand(torch.Size([768, 1, 1]))
        self.layer_scale27 = torch.rand(torch.Size([768, 1, 1]))
        self.layer_scale28 = torch.rand(torch.Size([768, 1, 1]))
        self.layer_scale29 = torch.rand(torch.Size([768, 1, 1]))
        self.layer_scale30 = torch.rand(torch.Size([768, 1, 1]))
        self.layer_scale31 = torch.rand(torch.Size([768, 1, 1]))
        self.layer_scale32 = torch.rand(torch.Size([768, 1, 1]))
        self.weight3 = torch.rand(torch.Size([768]))
        self.bias3 = torch.rand(torch.Size([768]))
        self.layer_scale33 = torch.rand(torch.Size([1536, 1, 1]))
        self.layer_scale34 = torch.rand(torch.Size([1536, 1, 1]))
        self.layer_scale35 = torch.rand(torch.Size([1536, 1, 1]))
        self.weight4 = torch.rand(torch.Size([1536]))
        self.bias4 = torch.rand(torch.Size([1536]))

    def forward(self, x):
        x0=x
        x1=self.conv2d0(x0)
        x2=x1.permute(0, 2, 3, 1)
        x3=self.weight0
        x4=self.bias0
        x5=torch.nn.functional.layer_norm(x2, (192,),weight=x3, bias=x4, eps=1e-06)
        x6=x5.permute(0, 3, 1, 2)
        x7=self.layer_scale0
        x8=self.conv2d1(x6)
        x9=torch.permute(x8, [0, 2, 3, 1])
        x10=self.layernorm0(x9)
        x11=self.linear0(x10)
        x12=self.gelu0(x11)
        x13=self.linear1(x12)
        x14=torch.permute(x13, [0, 3, 1, 2])
        x15=operator.mul(x7, x14)
        x16=stochastic_depth(x15, 0.0, 'row', False)
        x17=operator.add(x16, x6)
        x18=self.layer_scale1
        x19=self.conv2d2(x17)
        x20=torch.permute(x19, [0, 2, 3, 1])
        x21=self.layernorm1(x20)
        x22=self.linear2(x21)
        x23=self.gelu1(x22)
        x24=self.linear3(x23)
        x25=torch.permute(x24, [0, 3, 1, 2])
        x26=operator.mul(x18, x25)
        x27=stochastic_depth(x26, 0.014285714285714285, 'row', False)
        x28=operator.add(x27, x17)
        x29=self.layer_scale2
        x30=self.conv2d3(x28)
        x31=torch.permute(x30, [0, 2, 3, 1])
        x32=self.layernorm2(x31)
        x33=self.linear4(x32)
        x34=self.gelu2(x33)
        x35=self.linear5(x34)
        x36=torch.permute(x35, [0, 3, 1, 2])
        x37=operator.mul(x29, x36)
        x38=stochastic_depth(x37, 0.02857142857142857, 'row', False)
        x39=operator.add(x38, x28)
        x40=x39.permute(0, 2, 3, 1)
        x41=self.weight1
        x42=self.bias1
        x43=torch.nn.functional.layer_norm(x40, (192,),weight=x41, bias=x42, eps=1e-06)
        x44=x43.permute(0, 3, 1, 2)
        x45=self.conv2d4(x44)
        x46=self.layer_scale3
        x47=self.conv2d5(x45)
        x48=torch.permute(x47, [0, 2, 3, 1])
        x49=self.layernorm3(x48)
        x50=self.linear6(x49)
        x51=self.gelu3(x50)
        x52=self.linear7(x51)
        x53=torch.permute(x52, [0, 3, 1, 2])
        x54=operator.mul(x46, x53)
        x55=stochastic_depth(x54, 0.04285714285714286, 'row', False)
        x56=operator.add(x55, x45)
        x57=self.layer_scale4
        x58=self.conv2d6(x56)
        x59=torch.permute(x58, [0, 2, 3, 1])
        x60=self.layernorm4(x59)
        x61=self.linear8(x60)
        x62=self.gelu4(x61)
        x63=self.linear9(x62)
        x64=torch.permute(x63, [0, 3, 1, 2])
        x65=operator.mul(x57, x64)
        x66=stochastic_depth(x65, 0.05714285714285714, 'row', False)
        x67=operator.add(x66, x56)
        x68=self.layer_scale5
        x69=self.conv2d7(x67)
        x70=torch.permute(x69, [0, 2, 3, 1])
        x71=self.layernorm5(x70)
        x72=self.linear10(x71)
        x73=self.gelu5(x72)
        x74=self.linear11(x73)
        x75=torch.permute(x74, [0, 3, 1, 2])
        x76=operator.mul(x68, x75)
        x77=stochastic_depth(x76, 0.07142857142857142, 'row', False)
        x78=operator.add(x77, x67)
        x79=x78.permute(0, 2, 3, 1)
        x80=self.weight2
        x81=self.bias2
        x82=torch.nn.functional.layer_norm(x79, (384,),weight=x80, bias=x81, eps=1e-06)
        x83=x82.permute(0, 3, 1, 2)
        x84=self.conv2d8(x83)
        x85=self.layer_scale6
        x86=self.conv2d9(x84)
        x87=torch.permute(x86, [0, 2, 3, 1])
        x88=self.layernorm6(x87)
        x89=self.linear12(x88)
        x90=self.gelu6(x89)
        x91=self.linear13(x90)
        x92=torch.permute(x91, [0, 3, 1, 2])
        x93=operator.mul(x85, x92)
        x94=stochastic_depth(x93, 0.08571428571428572, 'row', False)
        x95=operator.add(x94, x84)
        x96=self.layer_scale7
        x97=self.conv2d10(x95)
        x98=torch.permute(x97, [0, 2, 3, 1])
        x99=self.layernorm7(x98)
        x100=self.linear14(x99)
        x101=self.gelu7(x100)
        x102=self.linear15(x101)
        x103=torch.permute(x102, [0, 3, 1, 2])
        x104=operator.mul(x96, x103)
        x105=stochastic_depth(x104, 0.1, 'row', False)
        x106=operator.add(x105, x95)
        x107=self.layer_scale8
        x108=self.conv2d11(x106)
        x109=torch.permute(x108, [0, 2, 3, 1])
        x110=self.layernorm8(x109)
        x111=self.linear16(x110)
        x112=self.gelu8(x111)
        x113=self.linear17(x112)
        x114=torch.permute(x113, [0, 3, 1, 2])
        x115=operator.mul(x107, x114)
        x116=stochastic_depth(x115, 0.11428571428571428, 'row', False)
        x117=operator.add(x116, x106)
        x118=self.layer_scale9
        x119=self.conv2d12(x117)
        x120=torch.permute(x119, [0, 2, 3, 1])
        x121=self.layernorm9(x120)
        x122=self.linear18(x121)
        x123=self.gelu9(x122)
        x124=self.linear19(x123)
        x125=torch.permute(x124, [0, 3, 1, 2])
        x126=operator.mul(x118, x125)
        x127=stochastic_depth(x126, 0.12857142857142856, 'row', False)
        x128=operator.add(x127, x117)
        x129=self.layer_scale10
        x130=self.conv2d13(x128)
        x131=torch.permute(x130, [0, 2, 3, 1])
        x132=self.layernorm10(x131)
        x133=self.linear20(x132)
        x134=self.gelu10(x133)
        x135=self.linear21(x134)
        x136=torch.permute(x135, [0, 3, 1, 2])
        x137=operator.mul(x129, x136)
        x138=stochastic_depth(x137, 0.14285714285714285, 'row', False)
        x139=operator.add(x138, x128)
        x140=self.layer_scale11
        x141=self.conv2d14(x139)
        x142=torch.permute(x141, [0, 2, 3, 1])
        x143=self.layernorm11(x142)
        x144=self.linear22(x143)
        x145=self.gelu11(x144)
        x146=self.linear23(x145)
        x147=torch.permute(x146, [0, 3, 1, 2])
        x148=operator.mul(x140, x147)
        x149=stochastic_depth(x148, 0.15714285714285714, 'row', False)
        x150=operator.add(x149, x139)
        x151=self.layer_scale12
        x152=self.conv2d15(x150)
        x153=torch.permute(x152, [0, 2, 3, 1])
        x154=self.layernorm12(x153)
        x155=self.linear24(x154)
        x156=self.gelu12(x155)
        x157=self.linear25(x156)
        x158=torch.permute(x157, [0, 3, 1, 2])
        x159=operator.mul(x151, x158)
        x160=stochastic_depth(x159, 0.17142857142857143, 'row', False)
        x161=operator.add(x160, x150)
        x162=self.layer_scale13
        x163=self.conv2d16(x161)
        x164=torch.permute(x163, [0, 2, 3, 1])
        x165=self.layernorm13(x164)
        x166=self.linear26(x165)
        x167=self.gelu13(x166)
        x168=self.linear27(x167)
        x169=torch.permute(x168, [0, 3, 1, 2])
        x170=operator.mul(x162, x169)
        x171=stochastic_depth(x170, 0.18571428571428572, 'row', False)
        x172=operator.add(x171, x161)
        x173=self.layer_scale14
        x174=self.conv2d17(x172)
        x175=torch.permute(x174, [0, 2, 3, 1])
        x176=self.layernorm14(x175)
        x177=self.linear28(x176)
        x178=self.gelu14(x177)
        x179=self.linear29(x178)
        x180=torch.permute(x179, [0, 3, 1, 2])
        x181=operator.mul(x173, x180)
        x182=stochastic_depth(x181, 0.2, 'row', False)
        x183=operator.add(x182, x172)
        x184=self.layer_scale15
        x185=self.conv2d18(x183)
        x186=torch.permute(x185, [0, 2, 3, 1])
        x187=self.layernorm15(x186)
        x188=self.linear30(x187)
        x189=self.gelu15(x188)
        x190=self.linear31(x189)
        x191=torch.permute(x190, [0, 3, 1, 2])
        x192=operator.mul(x184, x191)
        x193=stochastic_depth(x192, 0.21428571428571427, 'row', False)
        x194=operator.add(x193, x183)
        x195=self.layer_scale16
        x196=self.conv2d19(x194)
        x197=torch.permute(x196, [0, 2, 3, 1])
        x198=self.layernorm16(x197)
        x199=self.linear32(x198)
        x200=self.gelu16(x199)
        x201=self.linear33(x200)
        x202=torch.permute(x201, [0, 3, 1, 2])
        x203=operator.mul(x195, x202)
        x204=stochastic_depth(x203, 0.22857142857142856, 'row', False)
        x205=operator.add(x204, x194)
        x206=self.layer_scale17
        x207=self.conv2d20(x205)
        x208=torch.permute(x207, [0, 2, 3, 1])
        x209=self.layernorm17(x208)
        x210=self.linear34(x209)
        x211=self.gelu17(x210)
        x212=self.linear35(x211)
        x213=torch.permute(x212, [0, 3, 1, 2])
        x214=operator.mul(x206, x213)
        x215=stochastic_depth(x214, 0.24285714285714285, 'row', False)
        x216=operator.add(x215, x205)
        x217=self.layer_scale18
        x218=self.conv2d21(x216)
        x219=torch.permute(x218, [0, 2, 3, 1])
        x220=self.layernorm18(x219)
        x221=self.linear36(x220)
        x222=self.gelu18(x221)
        x223=self.linear37(x222)
        x224=torch.permute(x223, [0, 3, 1, 2])
        x225=operator.mul(x217, x224)
        x226=stochastic_depth(x225, 0.2571428571428571, 'row', False)
        x227=operator.add(x226, x216)
        x228=self.layer_scale19
        x229=self.conv2d22(x227)
        x230=torch.permute(x229, [0, 2, 3, 1])
        x231=self.layernorm19(x230)
        x232=self.linear38(x231)
        x233=self.gelu19(x232)
        x234=self.linear39(x233)
        x235=torch.permute(x234, [0, 3, 1, 2])
        x236=operator.mul(x228, x235)
        x237=stochastic_depth(x236, 0.2714285714285714, 'row', False)
        x238=operator.add(x237, x227)
        x239=self.layer_scale20
        x240=self.conv2d23(x238)
        x241=torch.permute(x240, [0, 2, 3, 1])
        x242=self.layernorm20(x241)
        x243=self.linear40(x242)
        x244=self.gelu20(x243)
        x245=self.linear41(x244)
        x246=torch.permute(x245, [0, 3, 1, 2])
        x247=operator.mul(x239, x246)
        x248=stochastic_depth(x247, 0.2857142857142857, 'row', False)
        x249=operator.add(x248, x238)
        x250=self.layer_scale21
        x251=self.conv2d24(x249)
        x252=torch.permute(x251, [0, 2, 3, 1])
        x253=self.layernorm21(x252)
        x254=self.linear42(x253)
        x255=self.gelu21(x254)
        x256=self.linear43(x255)
        x257=torch.permute(x256, [0, 3, 1, 2])
        x258=operator.mul(x250, x257)
        x259=stochastic_depth(x258, 0.3, 'row', False)
        x260=operator.add(x259, x249)
        x261=self.layer_scale22
        x262=self.conv2d25(x260)
        x263=torch.permute(x262, [0, 2, 3, 1])
        x264=self.layernorm22(x263)
        x265=self.linear44(x264)
        x266=self.gelu22(x265)
        x267=self.linear45(x266)
        x268=torch.permute(x267, [0, 3, 1, 2])
        x269=operator.mul(x261, x268)
        x270=stochastic_depth(x269, 0.3142857142857143, 'row', False)
        x271=operator.add(x270, x260)
        x272=self.layer_scale23
        x273=self.conv2d26(x271)
        x274=torch.permute(x273, [0, 2, 3, 1])
        x275=self.layernorm23(x274)
        x276=self.linear46(x275)
        x277=self.gelu23(x276)
        x278=self.linear47(x277)
        x279=torch.permute(x278, [0, 3, 1, 2])
        x280=operator.mul(x272, x279)
        x281=stochastic_depth(x280, 0.32857142857142857, 'row', False)
        x282=operator.add(x281, x271)
        x283=self.layer_scale24
        x284=self.conv2d27(x282)
        x285=torch.permute(x284, [0, 2, 3, 1])
        x286=self.layernorm24(x285)
        x287=self.linear48(x286)
        x288=self.gelu24(x287)
        x289=self.linear49(x288)
        x290=torch.permute(x289, [0, 3, 1, 2])
        x291=operator.mul(x283, x290)
        x292=stochastic_depth(x291, 0.34285714285714286, 'row', False)
        x293=operator.add(x292, x282)
        x294=self.layer_scale25
        x295=self.conv2d28(x293)
        x296=torch.permute(x295, [0, 2, 3, 1])
        x297=self.layernorm25(x296)
        x298=self.linear50(x297)
        x299=self.gelu25(x298)
        x300=self.linear51(x299)
        x301=torch.permute(x300, [0, 3, 1, 2])
        x302=operator.mul(x294, x301)
        x303=stochastic_depth(x302, 0.35714285714285715, 'row', False)
        x304=operator.add(x303, x293)
        x305=self.layer_scale26
        x306=self.conv2d29(x304)
        x307=torch.permute(x306, [0, 2, 3, 1])
        x308=self.layernorm26(x307)
        x309=self.linear52(x308)
        x310=self.gelu26(x309)
        x311=self.linear53(x310)
        x312=torch.permute(x311, [0, 3, 1, 2])
        x313=operator.mul(x305, x312)
        x314=stochastic_depth(x313, 0.37142857142857144, 'row', False)
        x315=operator.add(x314, x304)
        x316=self.layer_scale27
        x317=self.conv2d30(x315)
        x318=torch.permute(x317, [0, 2, 3, 1])
        x319=self.layernorm27(x318)
        x320=self.linear54(x319)
        x321=self.gelu27(x320)
        x322=self.linear55(x321)
        x323=torch.permute(x322, [0, 3, 1, 2])
        x324=operator.mul(x316, x323)
        x325=stochastic_depth(x324, 0.38571428571428573, 'row', False)
        x326=operator.add(x325, x315)
        x327=self.layer_scale28
        x328=self.conv2d31(x326)
        x329=torch.permute(x328, [0, 2, 3, 1])
        x330=self.layernorm28(x329)
        x331=self.linear56(x330)
        x332=self.gelu28(x331)
        x333=self.linear57(x332)
        x334=torch.permute(x333, [0, 3, 1, 2])
        x335=operator.mul(x327, x334)
        x336=stochastic_depth(x335, 0.4, 'row', False)
        x337=operator.add(x336, x326)
        x338=self.layer_scale29
        x339=self.conv2d32(x337)
        x340=torch.permute(x339, [0, 2, 3, 1])
        x341=self.layernorm29(x340)
        x342=self.linear58(x341)
        x343=self.gelu29(x342)
        x344=self.linear59(x343)
        x345=torch.permute(x344, [0, 3, 1, 2])
        x346=operator.mul(x338, x345)
        x347=stochastic_depth(x346, 0.4142857142857143, 'row', False)
        x348=operator.add(x347, x337)
        x349=self.layer_scale30
        x350=self.conv2d33(x348)
        x351=torch.permute(x350, [0, 2, 3, 1])
        x352=self.layernorm30(x351)
        x353=self.linear60(x352)
        x354=self.gelu30(x353)
        x355=self.linear61(x354)
        x356=torch.permute(x355, [0, 3, 1, 2])
        x357=operator.mul(x349, x356)
        x358=stochastic_depth(x357, 0.42857142857142855, 'row', False)
        x359=operator.add(x358, x348)
        x360=self.layer_scale31
        x361=self.conv2d34(x359)
        x362=torch.permute(x361, [0, 2, 3, 1])
        x363=self.layernorm31(x362)
        x364=self.linear62(x363)
        x365=self.gelu31(x364)
        x366=self.linear63(x365)
        x367=torch.permute(x366, [0, 3, 1, 2])
        x368=operator.mul(x360, x367)
        x369=stochastic_depth(x368, 0.44285714285714284, 'row', False)
        x370=operator.add(x369, x359)
        x371=self.layer_scale32
        x372=self.conv2d35(x370)
        x373=torch.permute(x372, [0, 2, 3, 1])
        x374=self.layernorm32(x373)
        x375=self.linear64(x374)
        x376=self.gelu32(x375)
        x377=self.linear65(x376)
        x378=torch.permute(x377, [0, 3, 1, 2])
        x379=operator.mul(x371, x378)
        x380=stochastic_depth(x379, 0.45714285714285713, 'row', False)
        x381=operator.add(x380, x370)
        x382=x381.permute(0, 2, 3, 1)
        x383=self.weight3
        x384=self.bias3
        x385=torch.nn.functional.layer_norm(x382, (768,),weight=x383, bias=x384, eps=1e-06)
        x386=x385.permute(0, 3, 1, 2)
        x387=self.conv2d36(x386)
        x388=self.layer_scale33
        x389=self.conv2d37(x387)
        x390=torch.permute(x389, [0, 2, 3, 1])
        x391=self.layernorm33(x390)
        x392=self.linear66(x391)
        x393=self.gelu33(x392)
        x394=self.linear67(x393)
        x395=torch.permute(x394, [0, 3, 1, 2])
        x396=operator.mul(x388, x395)
        x397=stochastic_depth(x396, 0.4714285714285714, 'row', False)
        x398=operator.add(x397, x387)
        x399=self.layer_scale34
        x400=self.conv2d38(x398)
        x401=torch.permute(x400, [0, 2, 3, 1])
        x402=self.layernorm34(x401)
        x403=self.linear68(x402)
        x404=self.gelu34(x403)
        x405=self.linear69(x404)
        x406=torch.permute(x405, [0, 3, 1, 2])
        x407=operator.mul(x399, x406)
        x408=stochastic_depth(x407, 0.4857142857142857, 'row', False)
        x409=operator.add(x408, x398)
        x410=self.layer_scale35
        x411=self.conv2d39(x409)
        x412=torch.permute(x411, [0, 2, 3, 1])
        x413=self.layernorm35(x412)
        x414=self.linear70(x413)
        x415=self.gelu35(x414)
        x416=self.linear71(x415)
        x417=torch.permute(x416, [0, 3, 1, 2])
        x418=operator.mul(x410, x417)
        x419=stochastic_depth(x418, 0.5, 'row', False)
        x420=operator.add(x419, x409)
        x421=self.adaptiveavgpool2d0(x420)
        x422=x421.permute(0, 2, 3, 1)
        x423=self.weight4
        x424=self.bias4
        x425=torch.nn.functional.layer_norm(x422, (1536,),weight=x423, bias=x424, eps=1e-06)
        x426=x425.permute(0, 3, 1, 2)
        x427=self.flatten0(x426)
        x428=self.linear72(x427)
        return [x428]

m = M().eval()
x = torch.randn(1, 3, 224, 224)
start = time.time()
output = m(x)
end = time.time()
print(end-start)
